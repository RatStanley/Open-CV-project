...
ret, V_value = cv2.threshold(V_Orgin, 235, 255, cv2.THRESH_BINARY)
...
rgb_mod = cv2.cvtColor(img_hsv_converted, cv2.COLOR_HSV2BGR)
# x = 0
# print(rgb_mod.shape)
#
# while x != 1:
#     cv2.imshow('img_show',rgb_mod)
#     key = cv2.waitKey(30)
while key != ord('q'):
    ...
    ret, R_value = cv2.threshold(rgb_mod[:, :, 2], RL, 255, cv2.THRESH_TOZERO)
    ret, G_value = cv2.threshold(rgb_mod[:, :, 1], GL, 255, cv2.THRESH_TOZERO)
    ret, B_value = cv2.threshold(rgb_mod[:, :, 0], BL, 255, cv2.THRESH_TOZERO)

    img_hsv_converted[:, :, 0] = H_value
    img_hsv_converted[:, :, 1] = S_value
    img_hsv_converted[:, :, 2] = V_value

    gray = cv2.bitwise_and(img_hsv_converted, img_hsv_converted, img_hsv_converted)
